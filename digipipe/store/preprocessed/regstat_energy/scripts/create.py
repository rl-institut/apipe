...
def process() -> None:
    ...
    demand = demand.set_index(["lau_code", "district"]).div(3.6).reset_index()

    # Filter: federal states
    demand_states = demand.loc[demand.lau_code.str.len() == 2].set_index(
        "lau_code"
    )
    demand_states.district = demand_states.district.str[2:]
    ...
    demand_districts.district = demand_districts.district.str[6:]
