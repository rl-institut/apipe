...
def add_electricity_gas_relation_constraints(model, relations):
    ...
    for index, row in relations.iterrows():
        ...
        equate_flows_by_keyword(
            model=model,
            keyword1=f"{esys_conf.settings.optimize.gas_key}-{suffix}",
            keyword2=f"{esys_conf.settings.optimize.el_key}-{suffix}",
...
if __name__ == "__main__":
    ...
    try:
        ...
        if esys_conf.settings.optimize.receive_duals:
            m.receive_duals()

        # save solver log to scenario specific location
        solve_kwargs = esys_conf.settings.optimize.solve_kwargs
        solve_kwargs["logfile"] = logfile.split("." + logfile.split(".")[-1])[0] + "_solver_log.log"
