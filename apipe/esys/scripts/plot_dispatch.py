...
plotted : str
    ...
    path where a new directory is created and
    the plots are saved
...
Description
-------------
The script creates dispatch plots based on plot_dispatch and
plot_dispatch_plotly functions in oemoflex.
The static plots are saved with a file format defined by the *plot_filetype*
variable in
``apipe/esys/esys/config/settings.yaml`` and the interactive plotly plots as
html-files in a new directory called dispatch within directory plotted.
...
def plot_dispatch_data(df, df_demand, bus_name):
    ...
    fig_plotly = plots.plot_dispatch_plotly(
        df=df, df_demand=df_demand, unit="W", colors_odict=COLOR_DICT
    )
    ...
    fig_plotly.write_html(
        ...
        # The files are much smaller now because a script tag containing
        # the plotly.js source
        # code (~3MB) is not included in the output anymore. It is refered
        # to plotlyjs via a
    ...
    for start_date, end_date in timeframe:
        ...
        # Simplify legend. As there is only one color per technology, there
        # should be only one label per technology.
        simple_labels_dict = {
        ...
        file_name = (
            ...
            + "_"
            + start_date[5:7]
            + esys_conf.settings.general.plot_filetype
        )
...
def aggregate_by_region(bus_files, carrier):
    ...
    bus_files: pd.DataFrame
        Dataframe with bus data from
        ``results/{scenario}/postprocessed/sequences/bus``
    ...
    if len(busses_to_be_aggregated) > 1:
        for bus_to_be_aggregated in busses_to_be_aggregated:
            df, df_demand, bus_name = prepare_dispatch_data(
                bus_to_be_aggregated
            )
...
if __name__ == "__main__":
    ...
    logger = esys_conf.add_snake_logger("plot_dispatch")
