...
from apipe.esys.esys.config.esys_conf import COLOR_DICT, LABELS, load_yaml
from apipe.esys.esys.tools import data_processing as dp

logger = esys_conf.add_snake_logger("data_processing")

MW_TO_W = 1e6
IGNORE_DROP_LEVEL = esys_conf.settings.plot_scalar_results.ignore_drop_level
...
def prepare_scalar_data(
    ...
    * drop near zero entries
    ...
    # define ordering and use concrete_order as keys for colors_odict in
    # plot_scalars
...
def draw_plot(df, unit, title):
    ...
    fig, ax = plt.subplots()
    plot_grouped_bar(ax, df, COLOR_DICT, unit=unit, stacked=True)
    ax.set_title(title)
...
def set_hierarchical_xlabels(
    ...
    r"""
    adapted from https://linuxtut.com/ 'Draw hierarchical axis labels with
    matplotlib + pandas'
    """
    ...
    if not isinstance(index, pd.MultiIndex):
        logging.info(
            "Index is not a pd.MultiIndex. "
            "Need a multiindex to set hierarchical labels."
        )
        return None
    ...
    if isinstance(rotation, (float, int)):
        ...
        raise ValueError(
            "Number of values for rotation "
            "must be 1 or match number of index levels."
        )

    if bar_yinterval is None:
        SPACE_PER_LETTER = 0.05
        bar_yinterval = _get_auto_bar_yinterval(
            index, SPACE_PER_LETTER, rotation
        )
    ...
    if isinstance(bar_yinterval, (float, int)):
        ...
        raise ValueError(
            "Must either pass one value for bar_yinterval or a "
            "list of values that matches the"
