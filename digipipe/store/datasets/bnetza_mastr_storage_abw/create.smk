...
DATASET_PATH = get_abs_dataset_path("datasets", "bnetza_mastr_storage_abw")
SOURCE_DATASET_PATH = get_abs_dataset_path("preprocessed", "bnetza_mastr", data_dir=True)

rule create:
    """
    Extract storages for ABW region
    """
    input:
    ...
    output:
        outfile=DATASET_PATH / "data" / "bnetza_mastr_storage_abw.gpkg",
        outfile_agg=DATASET_PATH / "data" / "bnetza_mastr_storage_agg_abw.gpkg"
    params:
